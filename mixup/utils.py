...
from mixup.models import BertConfig, MixupBertForSequenceClassification, LogitMixBertForSequenceClassification, RobertaConfig, LogitMixRobertaForSequenceClassification
...
class MixupTrainer(Trainer):
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if (
                    ...
                    # Mixup Transformer: modified
                    with model.no_sync():
                        tr_loss += self.training_step(model, inputs, epoch)
                else:
                    tr_loss += self.training_step(model, inputs, epoch)
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]], cur_epoch) -> torch.Tensor:
        model.train()
        inputs = self._prepare_inputs(inputs) # with batch size

        # Mixup Transformer: Different loss for different epochs
        loss = self.compute_loss(model, inputs, cur_epoch)
    ...
    def compute_loss(self, model, inputs, cur_epoch, return_outputs=False):
        ...
        # Mixup Transformer: Different loss for different steps
        ...
        with torch.no_grad():
            ...
            if has_labels:
                if isinstance(outputs, dict):
                    ...
                    loss = outputs[0]["loss"].mean().detach()
                    logits = outputs[0][1:]
            else:
                loss = None
                if isinstance(outputs, dict):
...
class MixupAutoModelForSequenceClassification:
    def __init__(self):
